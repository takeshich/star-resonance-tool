...
### Step 1: 画像のアップロード
使用するAI (ChatGPT, GPT-4o, Gemini 1.5 Pro 推奨) のチャット画面を開き、**「マスタ画像」** と **「モジュール画像」** をアップロードしてください。

> [!NOTE]
> チャットに一度に添付できる画像の枚数には上限がある場合があります。
> その場合、ゲーム内のフィルタ機能で **「攻撃」「防御」「支援」** などタイプ別に分け、数回に分けて解析を実行してください。
...
```text
あなたは高精度なゲームデータ解析AIです。添付した複数の画像を以下のルールで処理し、解析結果をコードブロックおよびダウンロード可能なCSV形式のみ出力してください。

【画像の種類と役割】
1. **マスタ画像**: 添付画像のうち、アイコンと「項目名（テキスト）」が一覧で書かれている画像があります。これを「辞書」として認識してください。
2. **ターゲット画像**: それ以外の画像は、読み取るべき「実際のモジュール」のスクリーンショットです。

【必須ルール：ゲーム仕様の厳守】 解析にあたり、以下の絶対的なルールを前提として認識してください。これに矛盾する解析結果は誤りです。

1.項目の重複禁止（ユニーク制約）:
- 1つのモジュール（1行）の中に、同じ項目（アイコン）が2回登場することは絶対にありません。
- もし視覚的に「同じアイコンが2つある」ように見えた場合は、あなたの見間違いです。必ずマスタ画像と詳細に照合し、細部の違い（盾の中の模様が「本」か「時計」か、エフェクトの形が違うか等）を見極め、それぞれ別の項目として特定してください。
- 数値を合算することは禁止です。
